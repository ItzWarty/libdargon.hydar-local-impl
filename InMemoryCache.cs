...
namespace Dargon.Hydar {
   public unsafe class InMemoryCache<K, V> : ICache<K, V> {
      ...
      public IDictionary<K, V> GetMany(IEnumerable<K> keys) {
         IDictionary<K, V> result = new Dictionary<K, V>();
         foreach (var key in keys) {
            result.Add(key, this.GetValueOrDefault(key));
         }
         return result;
      }
      ...
      IDictionary<K, V> ICache<K, V>.this[IEnumerable<K> keys] { get { return GetMany(keys); } }
      ...
      public bool Remove(KeyValuePair<K, V> item) {
         return ((ICollection<KeyValuePair<K, V>>)dict).Remove(item);
      }
